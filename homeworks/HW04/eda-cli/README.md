...
К существующему CLI-приложению для EDA добавлен **HTTP-сервис на FastAPI** с эндпоинтами `/health`, `/quality`
и `/quality-from-csv`.  
...
Эндпоинт принимает **агрегированные признаки датасета** (размеры, доля пропусков и т.п.) и возвращает эвристическую
оценку качества.
...
### 5. `POST /quality-flags-from-csv` – Получение набора флагов данных

Эндпоинт принимает CSV-файл и выдаёт булевые флаги, полученные из compute

Содержание ответа - булевы флаги:
...
### 6. `GET /metrics` – Получение статистики использования сервера

Содержание ответа:

- `total_requests` - Количество запросов на сервер, начиная с его запуска
- `total_latency_ms` - Среднее время ответа сервера
- `last_ok_for_model` - Качество последнего проверенного датасета (если данные ещё не загружались - `false`)
...
3. Проверить работу HTTP-сервиса (`uv run uvicorn ...`, затем `/health` и `/quality`/`/quality-from-csv` через `/docs`
   или HTTP-клиент).
