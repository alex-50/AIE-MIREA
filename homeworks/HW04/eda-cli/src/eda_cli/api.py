...
######## Реализация /metrics со статистикой по использованию сервера ########
...
async def metrics_middleware(request: Request, call_next):
    ...
    _METRICS_RUNTIME["total_requests"] += 1
    _METRICS_RUNTIME["total_latency_ms"] += latency_ms

    return response


class MetricsResponse(BaseModel):
    total_requests: int = Field(default=0, ge=0, description="Число выполненных запросов к серверу")
    avg_latency_ms: float = Field(default=0.0, ge=0.0, description="Среднее время ответа сервера в мс")
    last_ok_for_model: bool = Field(default=False, description="Результат последней проверки качества")
...
def metrics() -> MetricsResponse:
    """
    Получение статистики вызовов сервера
    """
    total = _METRICS_RUNTIME["total_requests"]

    avg_latency = (
        _METRICS_RUNTIME["total_latency_ms"] / total
        if total > 0 else 0.0
    )
    ...
    return MetricsResponse(
        ...
        last_ok_for_model=_METRICS_RUNTIME["last_ok_for_model"],
    )
...
def quality(req: QualityRequest) -> QualityResponse:
    ...
    ################### Для /metrics ##################
    _METRICS_RUNTIME["last_ok_for_model"] = ok_for_model
    ###################################################
...
########### Реализация возврата флагов качества датасета ###################

class QualityFlags(BaseModel):
    too_few_rows: bool = Field(..., description="Слишком мало строк")
    too_many_missing: bool = Field(..., description="Слишком много пропусков")
    has_constant_columns: bool = Field(..., description="Есть константные колонки")
    has_high_cardinality_categoricals: bool = Field(
        ..., description="Категориальные признаки с высокой кардинальностью"
    )
    has_suspicious_id_duplicates: bool = Field(
        ..., description="Подозрительные дубликаты ID"
    )


class CsvQualityFlagsResponse(BaseModel):
    flags: QualityFlags


@app.post(
    "/quality-flags-from-csv",
    response_model=CsvQualityFlagsResponse,
    tags=["quality"],
    summary="Получение набора флагов данных"
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> CsvQualityFlagsResponse:
    """
        Эндпоинт принимает CSV-файл и возвращает булевы флаги по его качеству для анализа
    """
    ...
    return CsvQualityFlagsResponse(flags=compute_quality_flags(summary, missing_df))
